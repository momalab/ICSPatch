...
int get_codesys_pid()
{
    ...
    while((dent = readdir(srcdir)) != NULL)
    {
        ...
        {
            snprintf(comm_path, sizeof(comm_path), "%s/%s/%s\0", "/proc", dent->d_name, "comm");
...
void get_inmemory_addresses(unsigned int tpid, long long *map_start_address, long long *map_end_address, long long *data_start_address, long long *data_end_address)
{
    char path[50];
    ...
    while(fgets(buffer, buffer_length, file_pointer)){
        ...
        *data_start_address = current_start_address;
        *data_end_address = current_end_address;

        //CHANGE: Change the address here to figure out the memory region start address for other devices
        if(current_start_address > 3055550464){
            break;
        }
...
int main(int argc, char const *argv[])
{
    ...
    long long data_start_address;
    long long data_end_address;
    ...
    while(1){
        ...
        if(cmd == COMM_QUIT){
            ...
            get_inmemory_addresses(tpid, &map_start_address, &map_end_address, &data_start_address, &data_end_address);
            printf("- Main task start address: %llu \n", map_start_address);
            printf("- Main task end address: %llu \n", map_end_address);

            char map_start_hexaddress[9];
            char map_end_hexaddress[9];
            char data_start_hexaddress[9];
            char data_end_hexaddress[9];
            ...
            sprintf(data_start_hexaddress, "%.8x\0", data_start_address);
            sprintf(data_end_hexaddress, "%.8x\0", data_end_address);

            printf("\n- Hex PID: %s\n", tpid_hexstr);
            printf("- Hex map start address: %s\n", map_start_hexaddress);
            printf("- Hex map end address: %s\n", map_end_hexaddress);
            printf("- Hex data start address: %s\n", data_start_hexaddress);
            printf("- Hex data end address: %s\n", data_end_hexaddress);

            //It sends a reply as hex string to ICSPatch: PID(Size: 4) + Map Start Address(Size: 8) + Map End Address(Size: 8) + Data Start Address(Size: 8) + Data End Address(Size: 8)
            sprintf(message, "%s%s%s%s%s\0", tpid_hexstr, map_start_hexaddress, map_end_hexaddress, data_start_hexaddress, data_end_hexaddress);
